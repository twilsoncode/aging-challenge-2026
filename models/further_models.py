...
def load_pseudobulk(split, cell_type="overall"):
    ...
    # Filter by cell type if requested and available in .obs (Keeps your original logic just in case)
    if cell_type != "overall" and 'cell_type' in adata.obs.columns:
        df = df[adata.obs['cell_type'] == cell_type]
        
    # Aggregate to donor level (sum counts)
    df_donor = df.groupby('donor_id').sum()
    
    # --- NEW: Filter columns to isolate specific cell type genes ---
    if cell_type != "overall":
        ct_safe = cell_type.replace(' ', '_')
        # Keep only columns that contain the cell type suffix
        cols_to_keep = [c for c in df_donor.columns if ct_safe in c or cell_type in c]
        if cols_to_keep:
            df_donor = df_donor[cols_to_keep]
